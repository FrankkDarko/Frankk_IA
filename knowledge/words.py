class Bonjour:
    demande = ["bonjour", "salut", "hello", "hi", "hey"]
    reponse = ["Salut", "Bonjour", "Hey", "Oh super content de te voir !"]


class Commandes:
    commandes = ["mdpgen", "8ball", "search", "pfc", "test", "exit", "sortie", "concombre", "help", "aide", "info",
                 "cryptor", "crypteur", "randomme", "rdme"]
    alias = ["gen", "oracle", "ppc", "recherche"]


class Humour:
    commandes = ["blague", "fais moi une blague", "raconte moi une blague"]


class Amabilite:
    demande = ["ça va ?", "comment ça va ?", "tu vas bien ?", "oui", "super", "non", "bof"]
    reponse = ["Bien et toi ?", "Je suis heureux que ça aille bien", "oh.... tu peux jouer si tu veux (fait : help)"]


class JeNeSuisPasUnRobot:
    demande = ["tu es un robot ?", "robot ?", "tu es une ia ?", "ia ?"]
    reponse = ["Je ne suis pas un robot !", "Je ne pense pas et toi ?"]
    rep_hum = ["oui", "non", "je ne suis pas un robot"]
    rep_bot = ["Dommage j'ai toujours rêver d'avoir un ami robot", "oh mon dieu un robot !"]


class ProfilIA:
    q_nom = ["comment t'appelles-tu ?", "quelle est ton nom ?", "tu t'appelles comment ?"]
    r_nom = ["Je m'appelle Frankk comme mon créateur", "Mon créateur ma nommée comme lui, Frankk"]
    q_age = ["quelle âge a tu ?", "tu à quelle âge ?"]
    r_age = []  # Réponse par infofkia
